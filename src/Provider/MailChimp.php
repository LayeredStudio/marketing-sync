...
class MailChimp implements ProviderInterface {
	...
	protected function apiRequest(string $path, array $args = []) {
		...
		$keyParts = explode('-', $options['apiKey']);
		$url = 'https://m:' . $options['apiKey'] . '@' . $keyParts[1] . '.api.mailchimp.com/3.0' . $path;

		$req = wp_remote_request($url, $args);

		return wp_remote_retrieve_json($req);
	}

	public function testConnection(): bool {
		$test = $this->apiRequest('/');

		return isset($test['account_id']) && $test['account_id'];
	}
	...
	public function getRequiredFields(): array {
		$fields = [
			'apiKey'	=>	[
				...
				'type'			=>	'text',
				'default'		=>	null,
				'description'	=>	__('API key can be found in your account on <strong>Account -> Extras -> API keys</strong> page.', 'marketing-sync')
			]
		];
	...
	public function getContactFields(array $connection): array {
		$fields = [[
			...
			'group'	=>	__('Member fields', 'marketing-sync')
		]];

		if (isset($connection['list']) && $connection['list']) {
			$mergeFields = $this->apiRequest('/lists/' . $connection['list'] . '/merge-fields');
			foreach ($mergeFields['merge_fields'] as $field) {
				$fields[] = [
					'id'	=>	$field['tag'],
					'name'	=>	$field['name'] . ' (' . $field['tag'] . ')',
					'group'	=>	__('Merge fields', 'marketing-sync')
				];
			}
		} else {
			$fields[] = [
				'id'	=>	'-',
				'name'	=>	__('Choose a list and save first', 'marketing-sync'),
				'group'	=>	__('Merge fields', 'marketing-sync')
			];
		}
	...
	public function syncUser(WP_User $user, array $connection, array $fields): bool {
		...
		$re = $this->apiRequest('/lists/' . $connection['list'] . '/members/' . md5(strtolower($user->user_email)), [
			'method'	=>	'PUT',
			'body'		=>	json_encode([
